...
export const seedDatabase = async (): Promise<void> => {
  try {
    ...
    const recipesWithAuthor = RECIPES.map((recipe) => ({
      ...recipe,
      author: authorId,
      likes: [createdUsers[1]._id, createdUsers[2]._id].slice(0, Math.floor(Math.random() * 3)),
      favorites: [createdUsers[1]._id].slice(0, Math.floor(Math.random() * 2)),
    }))

    const createdRecipes = await Recipe.create(recipesWithAuthor)
    console.log(`✓ 创建了 ${createdRecipes.length} 个食谱`)

    await User.findByIdAndUpdate(createdUsers[1]._id, {
      $set: {
        likedRecipes: [createdRecipes[0]._id, createdRecipes[1]._id],
        favoritedRecipes: [createdRecipes[1]._id],
        following: [authorId],
      },
    })
    await User.findByIdAndUpdate(createdUsers[2]._id, {
      $set: {
        likedRecipes: [createdRecipes[0]._id],
        following: [authorId],
      },
    })
    ...
    const postsWithData = POSTS.map((post, index) => ({
      ...post,
      author: createdUsers[index % createdUsers.length]._id,
      recipe: index < 2 ? createdRecipes[index % createdRecipes.length]._id : undefined,
      likes: createdUsers.filter((_, i) => i !== index % createdUsers.length).map((u) => u._id),
      comments: [
        {
    ...
    const createdPosts = await Post.create(postsWithData)
    console.log(`✓ 创建了 ${createdPosts.length} 个作品打卡`)
    ...
    const createdCourses = await Course.create(coursesWithAuthor)
    console.log(`✓ 创建了 ${createdCourses.length} 个课程`)

    console.log('种子数据初始化完成！')
    console.log('测试账号：')
    USERS.forEach((user) => {
      console.log(`  邮箱：${user.email}，密码：${user.password}`)
    })
  } catch (error) {
